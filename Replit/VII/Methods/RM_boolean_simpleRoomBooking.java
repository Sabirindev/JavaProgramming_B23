...
public class RM_boolean_simpleRoomBooking {
    public static void main(String[] args) {
        Scanner scan = new Scanner(System.in);
        System.out.println(simpleRoomBook(scan.nextBoolean(), scan.nextInt(), scan.nextInt(),scan.nextInt()));
        scan.close();

    }
    public static boolean simpleRoomBook(boolean isAvailable, int month, int day, int year){
        if ((month == 7 && day <=8 && day>=1 ) || year != 2018 || isAvailable == false){
            return false;
        }
        return true;
    }
